...
//Definindo a estrutura de dadows
...
//Instanciando objeto da estrutura
...
unsigned long tempo_entre_cliques = 0;
unsigned long tempoentreleituras_touch = 0;
...
int accexaustao, accpause, accstop, accstart;
...
float temp_backup, umi_backup, co2_backup;
...
//start = A //open = 1 FUNCIONANDO //reset = 4 //right = 8 FUNCIONANDO//setpoint = 0 FUNCIONANDO
//pause = 3  //return = B //left = C //program = 7 //menu = *
//stop = 2 FUNCIONANDO //down = 6 //ok = 9 //up = #
...
void setup() {
  ...
  if (esp_now_init() != ESP_OK) {
    Serial.println("Erro iniciando o ESP-NOW");
    return;
  }
  ...
  //Configurando o watch dog timer
...
void loop() {
  while (Serial2.available() >= 1) {
    ...
    if (recebido != ',' && recebido != '0' && recebido != '1' && recebido != '2' && recebido != '3' && recebido != '4' && recebido != '5' && recebido != '6' && recebido != '7' && recebido != '8' && recebido != '9' && recebido != '\n' && recebido != '\0') {
      ...
      if (recebido != '\n'/* && (posicao < 20 - 1)*/) {
        ...
        mensagem[posicao] = '\0';
        posicao = 0;
        sscanf(mensagem, "%ld,%ld,%ld,%ld,", &leitura_temp, &leitura_umi, &leitura_co2, &inutil);
        temp = float(leitura_temp) / 100.00;
        umi = float(leitura_umi) / 100.00;
        co2 = float(leitura_co2) / 1000000.00;
        Serial.println(temp);
        Serial.println(umi);
        Serial.println(co2);
  ...
  //DUPLO CLIQUE DE BOTOES
  if (millis() - tempo_entre_cliques > 3000) {
    if (accstart == 1) {
      accstop = 0;
    }
    if (accpause == 1) {
  ...
  if (key == 'A') {
    if (millis() - tempoentreleituras_touch > 500) {
      accstart = accstart + 1;
      tempoentreleituras_touch = millis();
    }
    if (accstart > 1) {
      Serial2.write('a');
      Serial2.write('\n');
      ...
      Serial2.write('a');
      Serial2.write('\n');
      appendFile(SD, "/d0.txt", "\r \n");
      appendFile(SD, "/d0.txt", "Botao iniciar pressionado");
      appendFile(SD, "/d0.txt", "\r \n");
      accstart = 0;
    }
  }
  ...
  if (key == '3') {
    ...
    if (accpause > 1) {
      ...
      Serial2.write('\n');
      appendFile(SD, "/d0.txt", "\r\n");
      appendFile(SD, "/d0.txt", "Botao pausa pressionado");
      appendFile(SD, "/d0.txt", "\r\n");
      accpause = 0;
    }
  }
  if (key == '2') {
    ...
    if (accstop > 1) {
      ...
      Serial2.write('\n');
      appendFile(SD, "/d0.txt", "\r\n");
      appendFile(SD, "/d0.txt", "Botao stop pressionado");
      appendFile(SD, "/d0.txt", "\r\n");
      accstop = 0;
    }
  }
  ...
  if (key == '1') {
    if (millis() - tempoentreleituras_touch > 250) {
      accexaustao = accexaustao + 1;
      tempoentreleituras_touch = millis();
    }
    if (accexaustao > 1) {
      Serial2.write('d');
      Serial2.write('\n');
      appendFile(SD, "/d0.txt", "\r\n");
      appendFile(SD, "/D0.txt", "Botao exaustao pressionado");
      appendFile(SD, "/d0.txt", "\r\n");
      accexaustao = 0;
    }
  }
  ...
  if (key == '8') { //tecla que aumenta o setpoint --FUNCIONANDO
  ...
  if (key == '9' && tela_setpoint == true) { //botao ira enviar os novos setpoints para a EEPROM
    ...
    appendFile(SD, "/d0.txt", "\r\n");
    appendFile(SD, "/d0.txt", "Setpoint alterado: ");
    setpoint_str = temp_int; setpoint_str += "."; setpoint_str += temp_dec; setpoint_str += "; ";
    setpoint_str = umi_int; setpoint_str += "."; setpoint_str += umi_dec; setpoint_str += "; ";
    setpoint_str = co2_int; setpoint_str += "."; setpoint_str += co2_dec; setpoint_str += "; ";

    setpoint_str.toCharArray(setpoint_char, 50);

    appendFile(SD, "/d0.txt", setpoint_char);
    appendFile(SD, "/d0.txt", "\r\n");
    ...
    setpoint_umi_int++;
    setpoint_co2_int++;
    msg = setpoint_temp_int; msg += ',';
    msg += setpoint_umi_int; msg += ',';
    msg += setpoint_co2_int; msg += ',';

    msg.toCharArray(bufferfinal, 30);
